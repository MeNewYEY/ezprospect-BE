...
def save_financials():
    input_data = request.json
    user_id = get_jwt_identity()
    input_data["prospect_id"] = Prospect.query.filter_by(
        user_id = user_id,
        account = input_data["account"]
    ).one_or_none().id
...
@app.route('/financials/<int:prospect_account>', methods=['GET'])
@jwt_required
def getStatements(prospect_account):
    user_id = get_jwt_identity()
    prospect = Prospect.query.filter_by(
        user_id = user_id,
        account = prospect_account
    ).one_or_none()
